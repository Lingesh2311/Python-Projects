...
parser.add_argument("--data_path", type=str, default="/save_files/",
                    metavar="PATH",
...
parser.add_argument("--g_steps", type=int, default=1, metavar="N",
                    help="Steps of generator updates in one round of\
                          adversarial training (defaul: 1)")
parser.add_argument("--d_steps", type=int, default=3, metavar="N",
                    help="Steps of discriminator updates in one round of\
                           adversarial training (defaul: 3)")
parser.add_argument("--gk_epochs", type=int, default=1, metavar="N",
                    help="Epochs of generator updates in one step of generate\
                           update (defaul: 1)")
parser.add_argument("--dk_epochs", type=int, default=3, metavar="N",
                    help="Epochs of discriminator updates in one step of\
                          generate update (defaul: 3)")
...
parser.add_argument("--n_rollout", type=int, default=16, metavar="N",
                    help="Number of rollouts (defaul: 16)")
...
parser.add_argument("--dis_lr", type=float, default=1e-3, metavar="LR",
                    help="Learning Rate of discriminator optimizer\
                          (defaul: 1e-3)")
...
def pretrain_generator(model_dict, optimizer_dict, scheduler_dict, dataloader,
    ...
    for i, sample in enumerate(dataloader):
        ...
        # sometimes smaller than 64 (16) is passed,
        # so this if statement disables it
        if (sample.size() == torch.zeros([64, 20]).size()):
...
def pretrain_discriminator(model_dict, optimizer_dict, scheduler_dict,
                           ...
                           negative_file, batch_size, epochs, use_cuda=False,
                           temperature=1.0):
    discriminator = model_dict["discriminator"]
    ...
    generate_samples(model_dict, negative_file,
                     batch_size, use_cuda, temperature)
    dis_dataloader_params["positive_filepath"] = positive_file
    dis_dataloader_params["negative_filepath"] = negative_file
    # this is where data iterator is used
...
def adversarial_train(model_dict, optimizer_dict, scheduler_dict,
                      ...
                      vocab_size, pos_file, neg_file, batch_size,
                      gen_train_num=1,
                      dis_train_epoch=5, dis_train_num=3, max_norm=5.0,
                      rollout_num=4, use_cuda=False, temperature=1.0, epoch=1,
                      tot_epoch=100):
    """
    ...
    for _ in range(gen_train_num):
        ...
        w_loss = loss_func("adv_worker")(all_goal, delta_feature_for_worker,
                                         ...
                                         use_cuda)
        # based on loss improve the parameters
        torch.autograd.grad(m_loss, manager.parameters())
        torch.autograd.grad(w_loss, worker.parameters())
        ...
        m_optimizer.step()
        w_optimizer.step()
        print("Adv-Manager loss: {:.5f} Adv-Worker loss: {:.5f}".format(m_loss,
              w_loss))
    ...
    for n in range(dis_train_epoch):
        ...
        """
        for d-steps do
            Use current G, θm,θw to generate negative examples and
        ...
        for _ in range(dis_train_num):
            for i, sample in enumerate(dataloader):
                ...
                loss.backward()
                d_lr_scheduler.step()
        print("{}/{} Adv-Discriminator Loss: {:.5f}".format(n,
                                                            range
                                                            (dis_train_epoch),
                                                            loss))
    # Save all changes
...
def save_checkpoint(model_dict, optimizer_dict,
                    scheduler_dict, ckpt_num, replace=False):
    file_name = "checkpoint" + str(ckpt_num) + ".pth.tar"
    torch.save({"model_dict": model_dict, "optimizer_dict": optimizer_dict,
                "scheduler_dict": scheduler_dict, "ckpt_num": ckpt_num},
               file_name)
    if replace:
...
def main():
    ...
    if checkpoint_path is not None:
        ...
        scheduler_dict = prepare_scheduler_dict(optimizer_dict, gamma=gamma,
                                                step_size=step_size)
    ...
    vocab_size = param_dict["leak_gan_params"]
    ["discriminator_params"]["vocab_size"]
    for i in range(param_dict["train_params"]["pre_dis_epoch_num"]):
        print("Epoch: {}/{}  Pre-Discriminator".format(i,
                                                       param_dict
                                                       ["train_params"]
                                                       ["pre_dis_epoch_num"]))
        model_dict, optimizer_dict, scheduler_dict = pretrain_discriminator(
            model_dict, optimizer_dict, scheduler_dict, dis_data_params,
            vocab_size=vocab_size, positive_file=pos_file,
            negative_file=neg_file, batch_size=batch_size, epochs=1,
            use_cuda=use_cuda)
    ckpt_num = 0
    ...
    for epoch in range(param_dict["train_params"]["pre_gen_epoch_num"]):
        print("Epoch: {}/{}  Pre-Generator".format(epoch,
                                                   param_dict["train_params"]
                                                   ["pre_gen_epoch_num"]))
        model_dict, optimizer_dict, scheduler_dict = pretrain_generator(
            model_dict, optimizer_dict, scheduler_dict, r_dataloader,
            vocab_size=vocab_size, use_cuda=use_cuda, epoch=epoch,
            tot_epochs=range(param_dict["train_params"]["pre_gen_epoch_num"]))
    ...
    vocab_size = param_dict["leak_gan_params"]["discriminator_params"]
    ["vocab_size"]
    # save checkpoint after this number of repetitions
    ...
    for epoch in range(param_dict["train_params"]["total_epoch"]):
        print("Epoch: {}/{}  Adv".format(epoch, param_dict["train_params"]
                                         ["total_epoch"]))
        model_dict, optimizer_dict, scheduler_dict = adversarial_train(
            model_dict, optimizer_dict, scheduler_dict, dis_data_params,
            vocab_size=vocab_size, pos_file=pos_file, neg_file=neg_file,
            batch_size=batch_size, use_cuda=use_cuda, epoch=epoch,
            tot_epoch=param_dict["train_params"]["total_epoch"])
        if (epoch + 1) % save_num == 0:
            ckpt_num += 1
            if ckpt_num % replace_num == 0:
                save_checkpoint(model_dict, optimizer_dict,
                                scheduler_dict, ckpt_num, replace=True)
            else:
                save_checkpoint(model_dict, optimizer_dict,
                                scheduler_dict, ckpt_num)
